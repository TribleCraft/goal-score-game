...
export function computeShotOutcome(
  ...
  const releaseDriftX = clamp(releaseX * 120, -0.28, 0.28);
  const releaseDriftY = clamp(releaseY * 95, -0.24, 0.28);
  const directionX = pullX + releaseDriftX;
  const directionY = pullY + releaseDriftY;
  const power = clamp(distancePx / smallerSide * 0.72 + releaseSpeed * 0.9 + averageSpeed * 0.24, 0.12, 1.82);
  const curve = clamp((releaseX * 160 - pullX * 0.35) * releaseSpeed * 0.55, -0.62, 0.62);
  const landingX = clamp(directionX * 9.8 + curve, -3.05, 3.05);
  const landingY = clamp(directionY * 7.1 - 1.52 + (power - 1) * 0.42, -1.72, 2.45);
